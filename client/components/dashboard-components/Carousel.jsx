...
const Carousel = ({ users, isLoading, currentUser, setCurrentUser }) => {
  ...
  const handleNext = () => {
    setCurrentIndex((prevIndex) =>
      prevIndex + viewable >= users.length
        ? users.length - 1
        : prevIndex + viewable
    );
  };
  ...
  return (
    <div className="flex w-full h-screen">
      <div className="flex flex-col text-sm bg-inherit border border-l-0 border-y-0 border-gray-500/50 h-full min-w-[200px]">
        ...
        <div className="p-1 pl-3 hover:bg-gray-500/30 mx-5 rounded-md">
          Search
        </div>
      ...
      <div className="flex flex-col">
        ...
        <div className="flex flex-col w-full h-full px-10 pt-10">
          ...
          <div className="flex gap-8 items-center">
            <div
              className="cursor-pointer scale-125 bg-white/30 hover:bg-white/40 rounded-md -translate-y-20"
            ...
            <div className="flex gap-6 w-[320px] md:w-[840px] lg:w-[1075px] h-[600px] overflow-x-hidden border border-x-0 border-b-0 border-slate-700/70">
              {users
                .slice(currentIndex, currentIndex + viewable)
                .map((user, index) => {
                  return (
                    <div
                      className="flex flex-col gap-2 pt-6 w-[250px]"
                      key={index}
                    >
                      ...
                      <div className="flex flex-col">
                        <div className="flex justify-between">
                          ...
                          {currentUser.iLiked.find(
                            (item) => item === user._id
                          ) ? (
                            <MdFavorite
                              onClick={() => handleUnlike(user)}
                              className="text-pink-600/80 hover:text-pink-500/80 cursor-pointer"
                              size={30}
                            />
                          ) : (
                            <MdFavoriteBorder
                              onClick={() => handleLike(user)}
                              className="text-gray-600/80 hover:text-gray-500/80 cursor-pointer"
                              size={30}
                            />
                          )}
                        </div>
                          ...
                          {user.interests.map((interest) => {
                            return (
                              ...
                              >
                                ...
                                <span>
                                  {
                                    list.find((item) => item.value === interest)
                                      ?.icon
                                  }
                                </span>
                              </div>
